...
run;
/*修改组织机构*/
%read_tablen(&path,组织架构fig,Sheet1,cust_fig);
%read_tablen(&path,组织架构fig,Sheet2,yyb_fig);
/*对客户修改*/
proc sql;
create table all_cust as
select
     ...
     b.city1
from all_cust a 
left join cust_fig b
on a.CONTRACT_NO=b.CONTRACT_NO;
quit;
data all_cust;
set all_cust;
if region_name1^='' then region_name=region_name1;
if branch_name1^='' then branch_name=branch_name1;
if city1^='' then city=city1;
if depart='网商贷事业部' then depart='小微贷事业部';/*网商的都归小微贷*/
if substr(region_name,1,4)="小微" then depart='小微贷事业部';
if substr(region_name,1,4)="农商" then depart='农商贷事业部';
if substr(region_name,1,2)="华" then depart='三农贷事业部';
drop region_name1 branch_name1 city1;
run;
/*对营业部修改*/
proc sql;
create table all_cust as
...
from all_cust a 
left join yyb_fig b
on a.depart=b.depart1 and a.branch_name=b.yyb1;
quit;
data all_cust;
set all_cust;
if region1^='' then region_name=region1;
if city1^='' then city=city1;
drop region1 city1 yyb1;
run;
/*重庆一部*/
%read_tablen(&path,重庆一部,Sheet1,cq_fig);
proc sql;
create table all_cust as
select
     a.*,
...
from all_cust a 
left join cq_fig b
on a.CONTRACT_NO=b._COL0;
quit;
data all_cust;
set all_cust;
if _col4^='' then branch_name=_col4;
drop _col4;
run;
data all_cust;
set all_cust;
...
/*con_amount*/
/*loan_date*/
/*cust_name CONTRACT_NO sys mon_pay total_pay periods_num con_yue prin_yue pay_day sh_pay_peri al_pay_period done sh_pay_sum al_pay_sum overdue_dt overdue_day overdue_pay last_pay*/
/*      CORP_PHONE*/
/*      SPO_NAME*/
/*	  SPO_PHONE*/
/*      SPO_CORP*/
...
/*data qiuli;*/
/*set all_cust;*/
/*drop wik	mon_pri	mon_int	mon_man	 MOBILE	ADDR	CORP_NAME	CORP_ADDR	CORP_PHONE	SPO_NAME	SPO_PHONE	SPO_CORP	SPO_CORP_ADDR	SPO_CORP_PHO	*/
/*    SPO_CORP_POS	CON_NAME1	CON_REL1	CON_PHO1	CON_ADDR1	CON_CORP_NAME1	CON_COM_POS1	CON_COM_PHO1	CON_NAME2	CON_REL2	CON_PHO2	CON_ADDR2	CON_CORP_NAME2	CON_CORP_POS2	*/
/*    CON_CORP_PHO2	CON_NAME3	CON_REL3	CON_PHO3	CON_ADDR3	CON_CORP_NAME3	CON_CORP_POS3	CON_CORP_PHO3;*/
/*run;*/
/*data cuishou;*/
...
/*run;*/
/* 输出数据源*/
/*PROC EXPORT DATA=liuaihua OUTFILE="E:\source_data\审批管理部数据源_&mytoday..xlsx" DBMS=EXCEL REPLACE LABEL;*/
/*RUN;*/
/*PROC EXPORT DATA=all_cust OUTFILE="E:\source_data\催收部数据源_&mytoday..xlsx" DBMS=EXCEL REPLACE LABEL;*/
/*RUN;*/
/*PROC EXPORT DATA=qiuli OUTFILE="E:\source_data\邱利需要的数据源_&mytoday..xlsx" DBMS=EXCEL REPLACE LABEL;*/
/*RUN;*/
