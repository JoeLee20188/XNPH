...
data my_pay_fuck;
set my_pay; 
if submit_date1='' or amount1='' or contract_no1='' then output;
run;
data my_pay;/*删除无效流水*/
set my_pay; 
...
/***************汇总每个合同号的日总*****************************/
proc sql;
create table my_pay1 as
select
a.submit_date1,a.contract_no1,sum(a.amount1) as amount2
from my_pay a
group by submit_date1,contract_no1;
quit;
data my_pay;
set my_pay1;
rename amount2=amount1;
run;
data my_pay1;
set my_pay;
...
select
a.submit_date,a.contract_no,sum(a.amount) as amount1
from it_pay1 a
group by submit_date,contract_no;
quit;
data it_pay1;
set it_pay;
...
PROC EXPORT DATA=it_delete OUTFILE="E:\source_data\it需要删除的流水1109.xlsx" DBMS=EXCEL REPLACE LABEL;
RUN;
PROC EXPORT DATA=it_add OUTFILE="E:\source_data\it需要增加的流水1109.xlsx" DBMS=EXCEL REPLACE LABEL;
RUN;
...
run;
PROC EXPORT DATA=my_it_check_2 OUTFILE="E:\source_data\财务需要核实的流水1104.xlsx" DBMS=EXCEL REPLACE LABEL;
RUN;
