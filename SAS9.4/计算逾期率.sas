...
%let today="31Aug2016"d;
%let mytoday=20160831;
%let yqday=&today.-30;
libname dat "E:\data";
...
round(sum(thirdaishou_yue)/10000,0.01) as fengmu
from all_M a
group by sys,depart,region_name,city,branch_name,sales_code;
quit;
/*空值标为0*/
data Overdue_Money;
set Overdue_Money;
...
round(M6_plus_sum/(fengmu-M6_plus_sum),0.0001) as M6_plus_Rate,
round(sum(M1_sum+M2_sum+M3_sum+M4_sum+M5_sum+M6_sum)/(fengmu-M6_plus_sum),0.0001) as Total_Rate
from Overdue_Money a
group by sys,depart,region_name,city,branch_name,sales_code;
quit;
/*输出*/
PROC EXPORT DATA=Depart_Rate OUTFILE="E:\source_data\分级逾期数据.xlsx" DBMS=EXCEL;
RUN;
